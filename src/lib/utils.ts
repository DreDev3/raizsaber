...
export const formatDuration = (durationInMs: number, showHours = false) => {
  ...
  const formatMember = (num: number) => num.toString().padStart(2, '0');

  if (hours > 0  || showHours) {
    return `${formatMember(hours)}:${formatMember(minutes)}:${formatMember(seconds)}`;
  }
  return `${formatMember(minutes)}:${formatMember(seconds)}`;
};
...
export const calculateInstallmentOptions = (price: number) => {
  const gatewayFeePercentage = 0.0399; //taxa do asaas 3,49%
  const gatwayFixedFee = 0.8; //taxa do asaas R$0,49
  ...
  for(let i = 1; i <= maxInstallments; i++) {
    ...
    if(i > noInterestInstallments) {
      total += total * gatewayFeePercentage + gatwayFixedFee;
    }
