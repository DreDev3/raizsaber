...
export const AdminCommentItem = ({ comment }: AdminCommentProps) => {
      ...
      <div className='flex items-center gap-4'>
        ...
        <div className='flex-1'>
          <div className='flex items-center gap-1.5 text-xs text-muted-foreground'>
            ...
            <span>•</span>
            <p>há {formatDistanceToNow(comment.createdAt, { addSuffix: true })}</p>
            <span>•</span>
            <Badge className='text-xs' variant={repliesCount === 0 ? 'destructive' : 'outline'}>
              {repliesCount === 0 ? 'Sem resposta' : `${repliesCount} resposta${repliesCount === 1 ? '' : 's'}`}
            </Badge>
          </div>
