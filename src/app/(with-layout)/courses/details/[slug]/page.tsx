...
export default async function CourseDetailsPage({ params }: CourseDetailsPageProps) {
  ...
  const totalDuration = course.modules.reduce((acc, mod) => {
    return acc + mod.lessons.reduce((lessonAcc, lesson) => lessonAcc + lesson.durationInMs, 0);
  }, 0);
  ...
  const details = [
    ...
    },
    {
      ...
      value: format(new Date(course.createdAt), 'dd/MM/yyyy'),
    }
  ];
  return (
      ...
      <div className='w-full grid md:grid-cols-[1fr_400px] gap-10'>
        <Tabs defaultValue='overview'>
          ...
          <TabsContent value='overview'>
            <EditorPreview className='opacity-90 mt-4' value={course.description} />
