...
export const POST = async (req: Request) => {
  try {
    ...
    if(token !== process.env.ASAAS_WEBHOOK_TOKEN) return new Response('Unauthorized', {status: 401});

    const body = await req.json();

    const {event, payment} = body;

    if(!event || !payment) return new Response('Bad Request', {status: 401});
